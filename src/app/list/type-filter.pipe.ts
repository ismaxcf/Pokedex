...
export class TypeFilteredPipe implements PipeTransform{
    transform(pokemons,searchTerm){
        ...
        if(typeof pokemons[0] === "string"){
            console.log(pokemons.filter(pokemon=>pokemon.toLowerCase().indexOf(searchTerm.toLowerCase())!==-1))
            return pokemons.filter(pokemon=>pokemon.toLowerCase().startsWith(searchTerm.toLowerCase(),0))
        }else{
